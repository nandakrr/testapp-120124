...
class BookingStepController extends Controller
{
    ...
    {
        $this->middleware('staging_bookingauth');
        $this->allDates = [];
        $this->ratingData = [];
    ...
    {
        $this->activeAuth = Session::get('staging_bookingAuth');
    }
    ...
    public function index()
    {
        ...
        // print_r(session()->get('ratingData'));
        // die;
        ...
        {
            return redirect('staging/booking/session/logout?do=token');
        }
        return view('staging.booking.index',compact('data','address_list','services_list','people_list','isRatingScreen','ratingRecords'));
    }
    ...
    {
        $address_list = $this->initAddress();
        $addressListRender = view('staging.booking.address.list',compact('address_list'))->render();
    ...
    public function savePeople(Request $request)
    {
        ...
        if ($json_result['ResponseMessage'] == 'SUCCESS') 
        {
            ...
            $addressListRender = view('staging.booking.people.list',compact('people_list'))->render();
    ...
    public function getBookingOptions(Request $request)
    {
        ...
        if ($json_result['ResponseMessage'] == 'SUCCESS') 
        {
            ...
            $response['message'] = 'address added successfully.'; 
            $response['content']['duration'] = view('staging.booking.booking-preferences.duration',compact('durations'))->render();
            if(session()->get('is_couple_gender') == 1)
            {
                ...
                $response['content']['gender']   = view('staging.booking.booking-preferences.couple-gender',compact('gender','activeGuest','selectedGuest','ppltLists'))->render();
            }
            else
            {
                $response['content']['gender']   = view('staging.booking.booking-preferences.gender',compact('gender'))->render();
            }
            $response['category_name']   = session()->get('category_name');
            $response['tooltip_text']   = session()->get('tooltip_text');
            $response['payload']   = $payload;
            //$response['content']['time_slots'] = view('staging.booking.booking-preferences.time-slots',compact('finalSlots'))->render();
        }
        else
        {
            $response['status'] = 422; 
    ...
    public function getDateOptions(Request $request)
    {
        ...
        if ($json_result['ResponseMessage'] == 'SUCCESS') 
        {
            ...
            $response['content']['date_slider'] = view('staging.booking.booking-details.date-slider',compact('startDate','endDate','number_dates'))->render();
            $response['content']['time_slots'] = view('staging.booking.booking-details.time-slots',compact('timeSlots'))->render();
    ...
    public function renderTimeBaseDate(Request $request)
    {
        ...
        if(isset($allDatesSlots[$selectedDate]))
        {
            ...
            $response['content']['time_slots_start'] = view('staging.booking.booking-details.time-slots',compact('timeSlots'))->render();

            $timeSlots = range(strtotime($loopStartTime),strtotime($DfltLoopEndTime),$step*60);
            $response['content']['time_slots_end'] = view('staging.booking.booking-details.time-slots',compact('timeSlots'))->render();
    ...
    public function summeryDetails(Request $request)
    {
        ...
        if ($json_result['ResponseMessage'] == 'SUCCESS') 
        {
            ...
            $response['content'] = view('staging.booking.summery.summery-details',compact('prices','netCost','parking','isCardSaved','cardSavedText'))->render();
            $response['response']['ResponseCode'] = $json_result['ResponseCode'];
        }
        else
        {
    ...
    public function stripeget(Request $request)
    {
        ...
        if ($json_result['ResponseMessage'] == 'SUCCESS') 
        {
            ...
            $response['url'] = 'booking/session/thank-you';
            return response()->json($response,$response['status']);
        }
        else
        {
    ...
    public function bookSession(Request $request)
    {
        ...
        if($response['status'] == 200) {
            $response['redirect_url'] = \URL::to('staging/booking/session/thank-you');
            return response()->json($response,$response['status']);
        }else {
            $returnCardList = $this->checkSavedCardResponse();
    ...
    public function logOut(Request $request)
    {
        ...
        {
            Session::forget('staging_bookingAuth');
            $this->distroyAllSession();
            ...
            if( isset($request->do) && $request->do == "token" )
            {
               ...
               return redirect('staging/booking/signin')->with('tokenmismatch', 'You have been logged out as you logged in on a different device');;
            }
